...
SINGLE_NODE_PROMPT = """请根据以下课程内容，生成口语化的课堂讲稿。
...
def call_llm_for_single_node(node_name: str, content: str) -> str:
    """为单个节点生成讲稿"""
    prompt = SINGLE_NODE_PROMPT.format(node_name=node_name, content=content[:2000])
    try:
        return call_llm(prompt)
    except Exception as e:
        log_print(f"LLM调用错误: {e}")
        return f"本节内容：{content[:200]}"


def _generate_fallback_script(node_name: str, teaching_content: str) -> str:
    """生成备用讲稿（不使用LLM）"""
    content = teaching_content or node_name or ""
    if not content:
        return "本节内容待补充。"
    if len(content) > 500:
        return content[:500] + "..."
...
def generate_script_for_node(node, use_llm: bool = False) -> str:
    """为单个CIR节点生成讲稿
    
    Args:
        node: CIRSection对象或类似对象，需有 node_name 和 teaching_content 属性
        use_llm: 是否使用LLM生成（默认False）
    
    Returns:
        str: 生成的讲稿内容
    """
    node_name = getattr(node, 'node_name', '') or ''
    teaching_content = getattr(node, 'teaching_content', '') or ''
    
    if use_llm and teaching_content:
        try:
            return call_llm_for_single_node(node_name, teaching_content)
        except Exception as e:
            log_print(f"LLM生成讲稿失败: {e}")
            return _generate_fallback_script(node_name, teaching_content)
    else:
        return _generate_fallback_script(node_name, teaching_content)


def batch_generate_for_lesson(lesson_id: str, db, use_llm: bool = False):
    """批量为课程的所有节点生成讲稿
    ...
    nodes = db.query(CIRSection).filter(
        CIRSection.lesson_id == lesson_id,
        CIRSection.node_type == "subchapter"
