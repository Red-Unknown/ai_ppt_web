...
def test_newcourse_associations():
    ...
    db = SessionLocal()
    try:
        # 查询现有的NewCourse记录（使用courses表）
        print("\n[前置准备] 查询现有数据")
        course = db.execute(text("SELECT id, title, subject_id, chapter_id FROM courses LIMIT 1")).fetchone()
        ...
        if course:
            ...
            # 创建测试科目
            subject = db.execute(text("INSERT INTO subjects (name) VALUES ('测试科目_关联') RETURNING id")).fetchone()
            subject_id = subject[0] if subject else None
            
            if subject_id:
                # 创建测试章节
                chapter = db.execute(text("INSERT INTO chapters (subject_id, name, \"order\") VALUES (:subject_id, '测试章节', 1) RETURNING id"), {"subject_id": subject_id}).fetchone()
                chapter_id = chapter[0] if chapter else None
                
                if chapter_id:
                    # 创建测试课程
                    course_result = db.execute(
                        text("INSERT INTO courses (title, subject_id, chapter_id, description) VALUES ('测试课程_关联', :subject_id, :chapter_id, '测试描述') RETURNING id"),
        ...
        doc = db.execute(
            text("INSERT INTO course_documents (course_id, title, content, doc_type) VALUES (:course_id, '测试文档', '测试内容', 'text') RETURNING id"),
            {"course_id": course_id}
        ).fetchone()
        db.commit()
        ...
        plan = db.execute(
            text("INSERT INTO lesson_plans (course_id, content) VALUES (:course_id, '{\"title\": \"测试教案\"}') RETURNING id"),
            {"course_id": course_id}
        ).fetchone()
        db.commit()
        ...
        # 测试6: 级联删除（模拟）
        print("\n[测试 6] 级联删除验证")
        print("  注意: 由于外键设置为CASCADE，删除课程时应自动删除关联数据")
